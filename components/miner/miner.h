...
/* BBB-AIRGAP: The miner API below must be called sequentially by a single owner task. start_miners()
 * leaves *ctx NULL if it could not start both miners, so a caller has to check it before using any
 * of the rest. */
