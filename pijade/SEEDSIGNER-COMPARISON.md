...
| **SettingsQR** (importing settings by QR) | separate flow | `settings_views.py:310` | **Absent.** There is no general settings import. QR configuration handles specific messages, including Blind Oracle configuration (`ui/dashboard.c:335`) and clock synchronisation (`qrmode.c:2737`, `:2770`) |
...
| **BBQr text import** | `main/bbqr.c`; the scanner branches on the file type at `qrmode.c:2902-2923` | Both sides read BBQr PSBTs; SeedSigner recognises type P (`models/decode_qr.py:366-367`, decoder at `:724`). This fork also routes type U text to its text import handlers (`qrmode.c:2915-2919`), including multisig setup files (`:2087-2092`). Reading only; this fork does not produce BBQr |
