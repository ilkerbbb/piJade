...
(`dashboard.c:3374`, filled circle = persistent, hollow = temporary) ; a difference in this fork's
...
| New seed ; **all sources at once** | Not in SeedSigner | **Present** | `Entropy Source > Combined` (`entropy_sources.c:227`): dice, camera frames and the device CSPRNG into one chain. On a build without a camera the row is named `Dice + Device` (`:229`) rather than silently dropping a source |
...
| **Combined entropy** | `Entropy Source > Combined` (`entropy_sources.c:227`) | Dice, camera frames and the device CSPRNG in one chain, so no single source has to be trusted |
