...
> **Line numbers reread 2026-09-15 at `01f266aa`.** Every `file:line` on the fork's side, the bare
> `:NNNN` ones included, was matched against the tree again and the stale ones corrected. What the
> cells themselves claim, which side has a feature and which does not, was not remeasured; that
> still stands from the 2026-09-13 reading above.
...
| 5 | **Sign message** | `seed_views.py:582`, guarded by `SETTING__MESSAGE_SIGNING == ENABLED` (**off** by default, `settings_definition.py:686-690`) | **Present, behind the same kind of switch** | `Session > <fingerprint> > Sign Message` (`dashboard.c:3457`), offered when `FEATURE_FLAGS_SIGN_MESSAGE` is set (`:3455`; `Options > Features`). The general QR scan is also gated (`qrmode.c:2058-2059`) and calls `sign_message_file()` (`:2002`, declaration at `:99`); the rpc path remains in the shared code |
| 6 | **BIP-85 child seed** | `seed_views.py:585`, guarded by `SETTING__BIP85_CHILD_SEEDS == ENABLED` (**off** by default, `settings_definition.py:663-668`) and `seed.bip85_supported` | **Present, and guarded the same way** | `dashboard.c:3463` -> `handle_bip85_mnemonic()`, offered when `FEATURE_FLAGS_BIP85` is set (`:3462`; `Options > Features`); 12 or 24 words (`ui/mnemonic.c:187-188`) |
| 7 | **Discard seed** | `seed_views.py:587` -> `SeedDiscardView:459`; the screen reads "Wipe seed {fingerprint} from the device?" (`:478`) | **Partly: temporary wallets only** | `Forget` at `dashboard.c:3479-3481`; no row appears for a persistent wallet, and the way to drop one is `Log Out` (`:3381`). The reason is written in the code (`:3475-3478`): `keychain_load()` refuses to read the blob back while a wallet is in memory |
...
(`dashboard.c:3376`, filled circle = persistent, hollow = temporary) ; a difference in this fork's
favour ; but there is no row for loading a new wallet from the list itself; that happens through
`Scan QR` or `Options > Add Wallet` (`dashboard.c:2806`).
...
|---|---|---|---|
| Load by scanning a SeedQR | `LoadSeedView:163` | **Present** | `Restore Wallet > Scan QR` (`ui/mnemonic.c:89`); `Scan SeedQR` sits on the home screen itself while the device is uninitialised (`dashboard.c:96`, `:3701`), and moves one level down into the `QR Mode` menu once a PIN wallet exists (`dashboard.c:103`, `ui/dashboard.c:155-157`) |
...
|   `-- QR Settings                       :2882  (QR Density, Frame Rate; ui/qrmode.c:261-271)
|-- Features                              :2832 -> :2518
...
| **QR code density** | ADVANCED / Medium | `settings_definition.py:597-603` | **Present.** `Preferences > QR Settings > QR Density` (`dashboard.c:2882`, `ui/qrmode.c:261-271`) offers Low / Medium / High (`qrmode.c:250-255`), persisted at `:1925-1926` and applied to PSBT and xpub QR sizes (`:226-247`). Default is Low here (`:200-208`), Medium there |
...
| **BIP-85 child seeds** on/off | ADVANCED / **off** | `:663-668` | **Present.** `Features > BIP85` (`dashboard.c:2476`), read where the row is laid out (`:3462`). Default is on here, off there |
| **Electrum seeds** on/off | ADVANCED / off | `:670-676` | **Absent** (so is the feature itself) |
| **Message signing** on/off | ADVANCED / **off** | `:686-690` | **Present.** `Features > Sign Msg` (`dashboard.c:2477`), read where the row is laid out (`:3455`). Default is on here, off there |
...
| **SettingsQR** (importing settings by QR) | separate flow | `settings_views.py:310` | **Absent.** There is no general settings import. QR configuration handles specific messages, including Blind Oracle configuration (`ui/dashboard.c:335`) and clock synchronisation (`qrmode.c:2742`, `:2775`) |
...
| **BBQr text import** | `main/bbqr.c`; the scanner branches on the file type at `qrmode.c:2912-2933` | Both sides read BBQr PSBTs; SeedSigner recognises type P (`models/decode_qr.py:366-367`, decoder at `:724`). This fork also routes type U text to its text import handlers (`qrmode.c:2925-2929`), including multisig setup files (`:2087-2092`). Reading only; this fork does not produce BBQr |
