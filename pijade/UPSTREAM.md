...
| `main/jade_assert.h` | +13 / -2 | Upstream file | `JADE_STATIC_ASSERT` is built on C11 `_Static_assert` instead of the `sizeof(char[1 - 2 * !(cond)])` idiom. That idiom is silent about the one input it cannot handle: a condition that is not a constant expression turns the array into a variable length array, the code compiles, and nothing is checked (see `main/wallet.c`); and `__wrap_abort()` is declared `noreturn`. On ESP32 that wrap is a link-time one (`-Wl,--wrap=abort` in `main/CMakeLists.txt`) and the compiler still sees stdlib's own noreturn `abort()`, but the amalgamated build rewrites `abort()` to this name with a macro (`main/amalgamated.c`), so left unmarked the declaration made every noreturn function that ends in `abort()` look to the compiler as if it falls off its end |
